...
export default function Settings() {
  ...
  return <AppShell title={isMalayalam ? "പ്രൊഫൈലും ക്രമീകരണങ്ങളും" : "Profile & settings"} eyebrow={isMalayalam ? "അക്കൗണ്ട് മുൻഗണനകൾ" : "Account preferences"} actions={<button className="primary-button compact" onClick={save} disabled={saving}><Save/>{saving ? (isMalayalam ? "സൂക്ഷിക്കുന്നു…" : "Saving…") : (isMalayalam ? "മാറ്റങ്ങൾ സൂക്ഷിക്കുക" : "Save changes")}</button>}><div className="settings-layout"><aside className="settings-nav"><button className={tab === "profile" ? "active" : ""} onClick={() => setTab("profile")}><UserRound/>{isMalayalam ? "പ്രൊഫൈൽ" : "Profile"}</button><button className={tab === "guidance" ? "active" : ""} onClick={() => setTab("guidance")}><ShieldCheck/>{isMalayalam ? "മാർഗരീതി" : "Guidance"}</button><button className={tab === "privacy" ? "active" : ""} onClick={() => setTab("privacy")}><LockKeyhole/>{isMalayalam ? "സ്വകാര്യത" : "Privacy"}</button><button className={tab === "notifications" ? "active" : ""} onClick={() => setTab("notifications")}><Bell/>{isMalayalam ? "അറിയിപ്പുകൾ" : "Notifications"}</button><button className={tab === "security" ? "active" : ""} onClick={() => setTab("security")}><KeyRound/>{isMalayalam ? "സുരക്ഷ" : "Security"}</button></aside>
    <section className="workspace-panel settings-card">
      {tab === "profile" && <><div className="settings-heading"><UserRound/><div><h2>{isMalayalam ? "വ്യക്തിഗത പ്രൊഫൈൽ" : "Personal profile"}</h2><p>{isMalayalam ? "നിങ്ങളുടെ സ്വകാര്യ പ്രവർത്തനകേന്ദ്രം ക്രമീകരിക്കാൻ ഉപയോഗിക്കുന്നു." : "Used to personalise your private workspace."}</p></div></div><div className="settings-fields"><label><span>{isMalayalam ? "പൂർണ്ണ പേര്" : "Full name"}</span><input value={form.fullName} onChange={event => setForm({ ...form, fullName: event.target.value })}/></label><label><span>{isMalayalam ? "അക്കൗണ്ട് റോൾ" : "Account role"}</span><select value={form.role} onChange={event => setForm({ ...form, role: event.target.value })}>{accountRoles.map(role => <option key={role}>{role}</option>)}</select></label><label><span>{isMalayalam ? "പ്രദേശം" : "Region"}</span><input value={form.region} onChange={event => setForm({ ...form, region: event.target.value })}/></label><label><span>{isMalayalam ? "ഭാഷ" : "Language"}</span><select value={form.language} onChange={event => setForm({ ...form, language: event.target.value })}><option>English</option><option>Malayalam</option><option>Arabic</option></select></label></div><div className="selected-role-settings"><ShieldCheck/><div><small>{isMalayalam ? selectedRole.malayalam : selectedRole.hubLabel}</small><strong>{isMalayalam ? selectedRole.greetingMalayalam : selectedRole.greeting}</strong><p>{isMalayalam ? selectedRole.descriptionMalayalam : selectedRole.description}</p><span>{(isMalayalam ? selectedRole.capabilitiesMalayalam : selectedRole.capabilities).map(item => <b key={item}><Check/>{item}</b>)}</span></div></div></>}
